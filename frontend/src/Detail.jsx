...
export default function Detail() {
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/toggle-favorite/${id}`, {
        method: "POST",
        headers: {
      ...
      if (data.success) {
        // ✅ update full favorites list
        setFavorites(data.favorites.map(favId => String(favId)));
      }
    } catch (err) {
  ...
  useEffect(() => {
    const fetchPlace = async () => {
      try {
        ...
        if (favData.success) {
          // ✅ ALWAYS store full array from DB
          setFavorites(favData.favorites.map(favId => String(favId)));
        }
      } catch (err) {
    ...
    <div className="min-h-screen bg-gray-100 font-sans pb-16 animate-fade-in flex flex-col">
      ...
      <div className="relative w-full h-[400px] md:h-[500px] overflow-hidden group">
        ...
        <button
          ...
          className={`absolute top-6 right-6 md:top-8 md:right-10 z-20 p-3 flex items-center justify-center rounded-full backdrop-blur-md transition-all duration-300 shadow-lg ${isFavorite
