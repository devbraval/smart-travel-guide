...
async function rankplaces(groups) {
  ...
  const prompt = `
Select the most famous places from each category.

Return format:
Category: place1, place2
...
`;
  ...
  try {
    ...
    text.split("\n").forEach(line => {
      ...
      const names = rest.split(",").map(n => normalize(n));

      const original = groups[cat.trim()] || [];

      result[cat.trim()] = original.filter(p =>
        names.some(n => normalize(p.name).includes(n))
      );

      /* fallback if AI returned nothing */
      if (result[cat.trim()].length === 0)
        result[cat.trim()] = original.slice(0,10);
