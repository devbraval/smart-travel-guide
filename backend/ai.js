...
async function processWithAi(places) {
  ...
  if (!process.env.GROQ_API_KEY || !Array.isArray(places) || places.length <= 10) {
    return places.map(p => p.name); // return NAMES only
  }

  const simplified = places.slice(0, 20).map(p => ({
    name: p.name,
...
From the following places, select the TOP tourist places.
...
`;
  ...
  try {
    ...
    if (!match) return places.map(p => p.name);

    const aiResult = JSON.parse(match[0]);
    const names = Object.values(aiResult.categories || {}).flat();

    return names.length ? names : places.map(p => p.name);
